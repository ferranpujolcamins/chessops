...
export class SquareSet implements Iterable<Square> {
  ...
  complement = () => new SquareSet(~this.lo, ~this.hi);

  xor = (other: SquareSet) => new SquareSet(this.lo ^ other.lo, this.hi ^ other.hi);

  union = (other: SquareSet) => new SquareSet(this.lo | other.lo, this.hi | other.hi);

  intersect = (other: SquareSet) => new SquareSet(this.lo & other.lo, this.hi & other.hi);

  diff = (other: SquareSet) => new SquareSet(this.lo & ~other.lo, this.hi & ~other.hi);

  intersects = (other: SquareSet) => this.intersect(other).nonEmpty();

  isDisjoint = (other: SquareSet) => this.intersect(other).isEmpty();

  supersetOf = (other: SquareSet) => other.diff(this).isEmpty();

  subsetOf = (other: SquareSet) => this.diff(other).isEmpty();
  ...
  bswap64 = () => new SquareSet(bswap32(this.hi), bswap32(this.lo));

  rbit64 = () => new SquareSet(rbit32(this.hi), rbit32(this.lo));
  ...
  equals = (other: SquareSet) => this.lo === other.lo && this.hi === other.hi;

  size = () => popcnt32(this.lo) + popcnt32(this.hi);

  isEmpty = () => this.lo === 0 && this.hi === 0;

  nonEmpty = () => this.lo !== 0 || this.hi !== 0;

  has = (square: Square) => (square >= 32 ? this.hi & (1 << (square - 32)) : this.lo & (1 << square)) !== 0;

  set = (square: Square, on: boolean) => (on ? this.with(square) : this.without(square));
  ...
  withoutFirst = () =>
    this.lo !== 0 ? new SquareSet(this.lo & (this.lo - 1), this.hi) : new SquareSet(0, this.hi & (this.hi - 1));

  moreThanOne = () =>
    (this.hi !== 0 && this.lo !== 0) || (this.lo & (this.lo - 1)) !== 0 || (this.hi & (this.hi - 1)) !== 0;

  singleSquare = (): Square | undefined => (this.moreThanOne() ? undefined : this.last());
